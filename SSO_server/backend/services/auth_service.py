...
def authorization(req):
    ...
    if session is not None and redirect_uri is not None:
        ...
        if not is_valid_exp(saved_session["expires_at"]):
            return jsonify({
                "error": "invalid_session",
                "error_description": "Expired session"
            }), HTTPStatus.BAD_REQUEST
        ...
        temp = execute_sql(
            ...
            (authentication_code, client_id, user_id, redirect_uri, scope, exp_at)
        )
        if not temp:
            return jsonify({
                "error": "internal_error",
    ...
    if redirect_uri not in redirect_uri:
        ...
        browser_session = secrets.token_urlsafe(64)
        browser_session_exp = datetime.now(timezone.utc) + timedelta(days=7)
        ...
        response_data = {
            ...
            "browser_session_exp": browser_session_exp
        }
        if state: 
            response_data["state"] = state
...
def exchange_token(req):
    ...
    browser_session = req_body.get("browser_session")
