...
def sso_callback():
    ...
    if auth_code:
        ...
        if response.status_code != 200:
            ...
            # SỬA LỖI TẠI ĐÂY:
            # 1. Khởi tạo biến data bằng một dictionary rỗng phòng trường hợp response không phải JSON.
            data = {}
            try:
                # 2. Lấy nội dung JSON từ response object
                data = response.json()
            except requests.exceptions.JSONDecodeError:
                # Xử lý nếu response không phải JSON (ví dụ: HTML/text lỗi)
        ...
        try:
            ...
            if not user_row:
                ...
                cursor.execute("""
                    INSERT INTO users (username, fullname, avatar)
                    VALUES (%s, %s, %s)
